...
def translate_single_file_local(
    ...
    print("Mode: Local Ollama (No Rate Limits)")
    print("=" * 60)
    ...
    # 4. Load translator (Ollama only)
    print(f"\n[4/7] Loading Ollama translator...")
    try:
    ...
    # 5. Translate chunks (NO DELAYS for local)
    print(f"\n[5/7] Translating {len(chunks)} chunks (no rate limits)...")
    print("-" * 60)
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        try:
            translated_text = translator.translate(user_content, system_prompt)
            previous_translation = translated_text
            translated_chunks[i] = translated_text
            print(f"\n[{i}/{len(chunks)}] ✓ Done — {len(result)} Myanmar chars" if 'result' in dir() else f"\n[{i}/{len(chunks)}] ✓ Done")
        except Exception as e:
...
Examples:
  ...
  python local_main.py --max-chars 1500
    ...
    try:
        translator = get_translator("ollama")
        print(f"✓ Ollama ready: {translator.name}")
    except Exception as e:
