...
Database-only architecture — all glossary reads/writes go through SQLite.
No JSON glossary files are used.
"""
...
class MemoryManager:
    """
    3-Tier Memory Management System (SQLite-only):
    - Tier 1: Per-novel Glossary (novel-specific, all writes go to DB)
    - Tier 2: Chapter Context (FIFO sliding window, stored in context_snapshots)
    ...
    def __init__(
        ...
        # Legacy params accepted but ignored — DB-only now
        glossary_path: str = "",
        context_path: str = "",
        ...
        logger.info(f"MemoryManager initialized (DB-only, novel={novel_name})")
    ...
    def _check_target_similarity(self, new_source: str, new_target: str,
        ...
        """
        terms = self.glossary_repo.get_terms_by_novel(self.novel_id, status="approved", limit=500)
        for term in terms:
            existing_target = term.get("target_term", "")
            existing_source = term.get("source_term", "")
    ...
    def update_term(self, source: str, new_target: str, chapter: int = 0) -> bool:
        ...
        self.glossary_repo.update_term(
            term["id"], target_term=new_target, canonical_form=new_target,
    ...
    def save_memory(self):
        """No-op — SQL writes are immediate on each operation."""
        pass
    ...
    def add_term(self, source: str, target: str, category: str = "general",
        ...
        logger.info(f"Added glossary term: {source} -> {target} (scope={scope})")
        self._invalidate_cache()
        return True
    ...
    def update_chapter_context(self, chapter_num: int, translated_text: str = "", summary: str = "", source_text: str = "") -> None:
        ...
        logger.info(f"Context updated for chapter {chapter_num}: {len(active_chars)} active chars, {len(events)} events")
