...
def run_ingest_human_correction(args: argparse.Namespace) -> int:
    """Diff .mm.md vs .human.md and extract corrections into glossary.

    For report.md §4 and §5: turns real human edits into system memory.
    Detects term replacements, voice/register changes, and style corrections.
    ...
    if not args.novel or not args.chapter:
        logger.error("--ingest-human-correction requires --novel and --chapter")
        return 1
    ...
    mm_path = output_dir / f"{novel}_chapter_{chapter:04d}.mm.md"
    human_path = output_dir / f"{novel}_chapter_{chapter:04d}.human.md"

    if not mm_path.exists():
        logger.error(f"AI translation not found: {mm_path}")
        return 1
    if not human_path.exists():
        logger.error(f"Human reference not found: {human_path}")
        return 1

    try:
        mm_text = FileHandler.read_text(str(mm_path))
        human_text = FileHandler.read_text(str(human_path))
        ...
        # Diff line by line
        mm_lines = mm_text.split("\n")
        human_lines = human_text.split("\n")

        corrections = []
        matcher = SequenceMatcher(None, mm_lines, human_lines)
        for tag, i1, i2, j1, j2 in matcher.get_opcodes():
            if tag == "replace":
                old = "\n".join(mm_lines[i1:i2])
                new = "\n".join(human_lines[j1:j2])
                # Only extract if meaningful change (not whitespace/markdown)
                if old.strip() and new.strip() and old.strip() != new.strip():
                    corrections.append({"old": old[:200], "new": new[:200], "type": "replacement"})
            elif tag == "insert":
                new = "\n".join(human_lines[j1:j2])
                if new.strip():
                    corrections.append({"new": new[:200], "type": "insertion"})
            elif tag == "delete":
                old = "\n".join(mm_lines[i1:i2])
                if old.strip():
                    corrections.append({"old": old[:200], "type": "deletion"})

        # Extract term pairs: look for name/term substitutions
        # Uses SequenceMatcher ratio to filter noise — only meaningful changes count
        term_corrections = []
        seen_pairs: set = set()
        for c in corrections[:50]:
            if c.get("type") != "replacement":
                continue
            old_text = c["old"]
            new_text = c["new"]
            # Skip if change is too small (likely particle/whitespace noise)
            if len(old_text) < 10 or len(new_text) < 10:
                continue
            # Skip if similarity > 0.85 (not a meaningful substitution)
            if SequenceMatcher(None, old_text, new_text).ratio() > 0.85:
                continue
            mm_words = set(re.findall(r'[\u1000-\u109F]{2,}', old_text))
            human_words = set(re.findall(r'[\u1000-\u109F]{2,}', new_text))
            added = human_words - mm_words
            removed = mm_words - human_words
            for a in list(added)[:2]:
                for r in list(removed)[:2]:
                    pair = (r, a)
                    if pair in seen_pairs:
                        continue
                    seen_pairs.add(pair)
                    mm.add_pending_term(
                        source=f"human-corrected: {r}",
                        target=a,
                        category="correction",
                        chapter=chapter,
                    )
                    term_corrections.append({"old": r, "new": a})
                    if len(term_corrections) >= 50:
                        break
                if len(term_corrections) >= 50:
                    break

        print(f"\n{'='*60}")
        print(f"  Human Correction Ingestion: {novel} #{chapter}")
        print(f"{'='*60}")
        print(f"  Lines compared: AI={len(mm_lines)}, Human={len(human_lines)}")
        print(f"  Corrections found: {len(corrections)}")
        print(f"  Term pairs extracted: {len(term_corrections)}")
        if term_corrections:
            print(f"\n  Term corrections added to pending glossary:")
            for tc in term_corrections[:10]:
                print(f"    {tc['old']} → {tc['new']}")
            if len(term_corrections) > 10:
                print(f"    ... and {len(term_corrections) - 10} more")
        print(f"{'='*60}\n")
        return 0
